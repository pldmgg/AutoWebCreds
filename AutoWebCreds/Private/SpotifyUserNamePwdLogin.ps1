function SpotifyUserNamePwdLogin {
    [CmdletBinding()]
    param(
    ...
    # Get UserName Field
    $UserNameField = Get-SeElement -By XPath -Selection '//*[@id="login-username"]' -Target $SeleniumDriver
    if (!$UserNameField) {
    ...
    $PwdField = Get-SeElement -By XPath -Selection '//*[@id="login-password"]' -Target $SeleniumDriver
    if (!$PwdField) {
    ...
    $ActuallyLoginButton = Get-SeElement -By XPath -Selection '//*[@id="login-button"]' -Target $SeleniumDriver
